...
check_template_contract() {
  ...
  grep -q "Env Alias Migration Gate (Required)" "${TEMPLATE_PATH}" \
    || fail "Template must include 'Env Alias Migration Gate (Required)' section."

  grep -q "docs/reports/ENV_ALIAS_MIGRATION_AUDIT_YYYY-MM-DD\.md" "${TEMPLATE_PATH}" \
    || fail "Template must include required audit artifact placeholder path."

  grep -q 'Checkpoint status:[[:space:]]*`PASS`' "${TEMPLATE_PATH}" \
    || fail "Template must include 'Checkpoint status' field set to PASS."

  grep -q 'Allowlist guard status:[[:space:]]*`PASS`' "${TEMPLATE_PATH}" \
...
check_ticket_file() {
  ...
  audit_ref="$(grep -E -o "docs/reports/ENV_ALIAS_MIGRATION_AUDIT_[0-9]{4}-[0-9]{2}-[0-9]{2}\.md" "${ticket_path}" | head -n 1 || true)"
  ...
  grep -E -q 'Checkpoint status:[[:space:]]*`?PASS`?' "${ticket_path}" \
    || fail "Cutover ticket must include 'Checkpoint status: PASS'."

  grep -E -q 'Allowlist guard status:[[:space:]]*`?PASS`?' "${ticket_path}" \
