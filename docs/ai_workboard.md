...
| --------------- | ---------- | ------------------------------------------ | ----------- | -------------------------------------------------------------------------------------------------- |
| T-2026-06-06-REAUDIT-APPCHECK-CSP | 2026-06-06 | Re-Audit Gate 0 P0.2 — web App Check + CSP | Completed | App Check (reCAPTCHA v3 + debug token + auto-refresh) initialized; CSP now allows *.cloudfunctions.net + reCAPTCHA origins. 13 tests. Needs env: NEXT_PUBLIC_FIREBASE_APPCHECK_RECAPTCHA_KEY (+ optional DEBUG_TOKEN). Next Gate 0: rules-emulator harness, domain decision. |
