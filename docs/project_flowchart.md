...
- **2026-03-13 Discovery legacy-web compatibility:** Firestore trigger `syncLegacyDiscoveryFields` now mirrors canonical nested discovery fields back onto the legacy flat root fields on `users/{uid}` so stale Firestore-only web clients continue to surface newly eligible users immediately until the public web deployment fully cuts over to the backend deck.
