...
```dart
String userId = ...;
ExampleConnector.instance.getPostsByUser(
  userId: userId,
).execute();
```
...
#### Return Type
`execute()` returns a `QueryResult<GetPostsByUserData, GetPostsByUserVariables>`
```dart
...
final ref = ExampleConnector.instance.getPostsByUser(
  userId: userId,
).ref();
ref.execute();
...
```dart
String postId = ...;
ExampleConnector.instance.likePost(
  postId: postId,
).execute();
```
...
final result = await ExampleConnector.instance.likePost(
  postId: postId,
);
...
final ref = ExampleConnector.instance.likePost(
  postId: postId,
).ref();
ref.execute();
```
