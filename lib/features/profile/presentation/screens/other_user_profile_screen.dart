...
class OtherUserProfileScreen extends StatelessWidget {
  ...
  Future<void> _handlePass(BuildContext context, Profile profile) async {
    ...
    final isDeckProfile = _isCurrentDeckProfile(
      discoveryBloc.state,
      profile.id,
    );
    ...
    if (isDeckProfile) {
      discoveryBloc.add(
        DiscoverySwipedLeft(userId: userId, targetUserId: profile.id),
      );
      if (context.mounted) {
  ...
  Future<void> _handleLike(BuildContext context, Profile profile) async {
    ...
    if (isDeckProfile) {
      discoveryBloc.add(
        DiscoverySwipedRight(userId: userId, targetUserId: profile.id),
      );
      if (context.mounted) {
        _returnToDeck(context, preferPop: true);
      }
    ...
    try {
      ...
      if (context.mounted) {
        showErrorSnackBar(
          context,
          'Could not like this profile. Please try again.',
        );
      }
    }
  }
      ...
      builder: (sheetContext) {
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            return Padding(
              padding: EdgeInsetsDirectional.only(bottom: viewInsets),
              child: Container(
          ...
          children: [
            ...
            ListTile(
              ...
              onTap: () async {
                ...
                await safetyCubit.toggleMuteMessages(
                  profile.id,
                  mute: !isMuted,
                );
                if (context.mounted) {
